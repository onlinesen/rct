...
class GFXTest(object):
    ...
    def installbundle(self, serial):
        try:
            ...
            if "No such" in out:
                ...
                cpu = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
                cpu = cpu.strip()
                cmd = 'adb -s ' + serial + ' shell getprop ro.build.version.sdk'
                sdk = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
                sdk = sdk.strip()
                os.system(
                    'adb ' + ' -s ' + serial + ' push ' + os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap.so' + ' /data/local/tmp/')
                os.system(
                    'adb ' + ' -s ' + serial + ' push ' + os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap' + ' /data/local/tmp/')
                cmd = 'adb -s ' + serial + ' shell chmod 777 /data/local/tmp/minicap'
                subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                os.system(
                    'adb ' + ' -s ' + serial + ' push ' + os.getcwd() + '/lib/' + cpu + '/minitouch' + ' /data/local/tmp/')
    ...
    def testLaunch(self):
        try:
            ...
            try:
                tm = self.learning_by_modle((int(x), int(y)), os.getcwd() + "/lib/bundle/linear_svcClf_train_model.m")
                (path, shotname) = os.path.split(tm)
                tms = shotname.replace(".png","").split("_")
                start = str(tms[0]) + "." + str(tms[1])
                self.textout.insert(1.0, "Test Minicap End!\n ")
                self.textout.insert(1.0, "START TIME:" + str(tm) + u" 秒 \n ")
                self.textout.update()
            except Exception, e:
                pass
    ...
    def get_test_image(self):
        image_list = []
        label_list = []
        image_dir = os.getcwd() + '/pic/'
        file_names = sorted((fn for fn in os.listdir(image_dir) if fn.endswith(".png")), key=self.numerical_sort)
        file_names = [image_dir + fn for fn in file_names]
        for image_path in file_names:
            img = Image.open(image_path)
            w, h = img.size
            img.thumbnail((w // 8, h // 8))
            # print("processing...", image_dir, image_path)
            image_list.append(np.asarray(img).flatten())
            label_list.append(image_path)
        return image_list, label_list

    def learning_by_modle(self, xy, model):
        model = os.getcwd() + "/lib/bundle/linear_svcClf_train_model.m"
        clf = sklearn.externals.joblib.load(model)
        tst_img, label_img = self.get_test_image()
        # tr_img, tr_label = get_train_image_datas()
        getf = ""
        predicts = clf.predict(tst_img)
        for index, result in enumerate(predicts):
            if result == "started":
                print "get end image:", label_img[index]
                getf = label_img[index]
                break

        for i in xrange(0, len(label_img)):
            img = Image.open(label_img[i])
            color = img.getpixel(xy)
            get = GetMainColor.min_color_diff(color)
            # print "get color :",get[1]
            if get[1] in ("VIOLET", "RED", "ORANGE"):
                (path, shotname) = os.path.split(label_img[i])
                tms = shotname.replace(".png", "").split("_")
                start0 = float(str(tms[0]) + "." + str(tms[1]))
                print "get first image:", label_img[i]
                break

        (path, shotname) = os.path.split(getf)
        tms = shotname.replace(".png", "").split("_")
        start1 = float(str(tms[0]) + "." + str(tms[1]))
        st = round((start1 - start0), 3)
        print "start time :", st
        return st
            ...
            for index, line in enumerate(playcontent):
                if line != "":
                    ...
                    if loopflag and "end" not in line:
                        ...
                        # imgsrc = Image.open(self.d.screenshot(str(self.d.server.adb.device_serial()) + "_tmp.png"))
                        imgsrc = self.minicap_ins.crop_image()
                        print "click image resoult:", getORno
                        if imgsrc == None:
                            continue
                        # imgsrc = Image.open(self.d.screenshot("tmp.png"))
                        draw = ImageDraw.Draw(imgsrc)
        ...
        try:
            ...
            for i in xrange(0, len(serial)):
                ...
                if "encryptString" in r.content:
                    ...
                    out = self.raw_cmd('shell', 'setprop', 'persist.journey.ts', str(ency))
    ...
    def parserNode(self, nc, node, x, y):
        for i in node:
            bounds = i.getAttribute('bounds').strip()
    ...
    def study_mode_threading(self):
        ...
        if self.study_mode_flag:
            ...
            tkinter.messagebox.askokcancel('提示', '界面获取模式（取消需再次点击获取按钮）')
...
if __name__ == "__main__":
    test = GFXTest()
    test.gettk()
    # test.playatxfile("E:\work\GFXTest\GFXTest_3.4.5\temp.txt","10",3)
    # test.find_template(d=Device(), img=r"E:\work\GFXTest\GFXTest_3.4.5\3.1080x2160.png", wt=10)
