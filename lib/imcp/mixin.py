...
import cv2
import aircv as ac
from lib.imcp import base
from lib.imcp.base import nameddict
from lib.imcp import imutils
from lib.imcp import Pattern, Bounds, FindPoint
...
class DeviceMixin(object):
    def __init__(self, d):
        self.image_match_method = 'template'
        self.image_match_threshold = 0.7
    ...
    def pattern_open(self, image):
        if self.__screensize is None:
            self.__screensize = [int(self.d.info["displayWidth"]), int(self.d.info["displayHeight"])]
    ...
    def wait(self, pattern, timeout=10.0, safe=False, **match_kwargs):
        ...
        t = time.time() + timeout
        print "pattern:", pattern, match_kwargs
        while time.time() < t:
            print "Check Image..",pattern
            ret = self.exists(pattern, **match_kwargs)
            if ret:
                return ret
            time.sleep(0.5)
        if not safe:
            print 'Not found image %s' % (pattern,)
    ...
    def wait_gone(self, pattern, timeout=10.0, safe=False, **match_kwargs):
        ...
        while time.time() < t:
            ...
            time.sleep(0.2)
        if not safe:
            print ('Image not gone %s' % (pattern,))
    ...
    #     """
    #     if x < 1 and y < 1:
    #         display = [int(self.d.info["displayWidth"]),int(self.d.info["displayHeight"])]
    ...
    def _match_auto(self, screen, search_img, threshold):
        ...
        if ret and ret['confidence'] > threshold:
            return FindPoint(ret['result'], ret['confidence'], 'template', matched=True)
        ...
        if 1.0 * matches / total > 0.5:  # FIXME(ssx): sift just write here
            return FindPoint(ret['result'], ret['confidence'], 'sift', matched=True)
        return None
    ...
    def match(self, pattern, screen=None, rect=None, offset=None, threshold=None, method=None):
        ...
        matched = False
        match_method = "sift"
        print "threadhold:",threshold
        position = None
        print "compare method:",match_method
        if match_method == 'template':  # IMG_METHOD_TMPL
            ret = ac.find_template(screen, search_img)
            if ret is None:
                return None
    ...
    def screenshot(self, filename=None):
        ...
        try:
            ...
            path = self.d.screenshot(os.getcwd() + "/tmp.png")
            print "path=",path
            screen = Image.open(path)
        except IOError:
            # try taks screenshot again
            print ("warning, screenshot failed [2/1], retry again")
            path = self.d.screenshot(os.getcwd() + "/tmp.png")
            screen = Image.open(path)
    ...
    def click_image(self, pattern, timeout=20.0, action='click', safe=False, desc=None, delay=None, **match_kwargs):
        ...
        """
        pattern = self.pattern_open(pattern)
        print ('click image:%s %s', desc or '', pattern)
        start_time = time.time()
        found = False
        point = None
        while time.time() - start_time < timeout:
            ...
            if point is None:
                ...
                print('confidence: %s', point.confidence)
            if not point.matched:
                print('Ignore confidence: %s', point.confidence)
                continue
...
if __name__ == '__main__':
    ...
    d=Device()
    dm = DeviceMixin(d)
    getORno = dm.wait(r"E:\\work\\GFXTest\\DeviceAdminApp_VBbag1.720x1440.png", timeout=10)
    print getORno
